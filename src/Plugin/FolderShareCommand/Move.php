 ...
 *  description     = @Translation("Move files and folders"),
 *  category        = "copy & move",
 *  weight          = 30,
 *  parentConstraints = {
 *    "kinds"   = {
 ...
 * )
 */
class Move extends CopyMoveBase {
  ...
  public function getDescription(bool $forPage) {
    ...
    //
    // - Dialog: The description is longer and has the form "Move OPERAND
    ...
    if (count($selectionIds) === 1) {
      ...
      if ($forPage === TRUE) {
        ...
        return t(
          'Move this folder to a new location, including all of its contents.');
      }
      ...
      return t(
        'Move "@name" to a new location, including all of its contents.',
        [
          '@name' => $item->getName(),
        ]);
    }
    ...
    if (isset($selectionKinds[FolderShare::FOLDER_KIND]) === FALSE) {
      return t(
        'Move these @operand to a new location.',
        [
          '@operand' => $operand,
        ]);
    }
    ...
    return t(
      'Move these @operand to a new location, including all of their contents?',
      [
        '@operand' => $operand,
      ]);
  }
  ...
  public function getTitle(bool $forPage) {
    ...
    //
    // - Dialog: The title is short and has the form "Move OPERAND",
